            ...
            try
            {
                ...
                if (max >= len && len != 2048)
                {
                    ...
                    for (var i = 0; i < channel.Size; ++i)
                    {
                        ...
                        hash = link_item.Id % channel.Size;

                        current.WriterLinks[hash] = link_item;
                        current.Storage[hash] = buffer_item;
                    }
                ...
                hash = id % current.Size;

                var link = current.WriterLinks[hash];
                var buffer = current.Storage[hash];
